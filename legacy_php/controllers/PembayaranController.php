...
class PembayaranController
{
    ...
    public static function getAllPembayaranForAdmin(?string $searchTerm = null, string $orderBy = 'p.created_at DESC')
    {
        try {
            self::checkRequiredModelsAndMethods(['Pembayaran' => ['findAllWithKodePemesanan']]);
            // Teruskan parameter pencarian dan pengurutan ke Model
            $data = Pembayaran::findAllWithKodePemesanan($searchTerm, $orderBy);
            return $data ?: [];
        } catch (Exception $e) {
