...
// Ambil kata kunci pencarian dari URL (jika ada) dan bersihkan
$search_term = isset($_GET['search']) ? trim($_GET['search']) : null;
// === AKHIR PENANGANAN PENCARIAN ===

// 6. Ambil semua data alat sewa (dengan atau tanpa filter pencarian)
$daftar_alat_sewa = [];
$error_alat_sewa = null;

try {
    // Teruskan search_term ke controller
    $daftar_alat_sewa = SewaAlatController::getAll($search_term);
...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        <form action="<?= e(ADMIN_URL . '/alat_sewa/kelola_alat.php') ?>" method="GET" class="row g-3 align-items-center">
            <div class="col">
                <div class="input-group">
                    <span class="input-group-text bg-light"><i class="fas fa-search"></i></span>
                    <input type="text" name="search" class="form-control" placeholder="Cari berdasarkan Nama Item atau Kategori..." value="<?= e($search_term ?? '') ?>">
                </div>
            </div>
            <div class="col-auto">
            ...
            <?php if ($search_term): ?>
            <div class="col-auto">
                <a href="<?= e(ADMIN_URL . '/alat_sewa/kelola_alat.php') ?>" class="btn btn-outline-secondary">Reset</a>
            </div>
            <?php endif; ?>
        </form>
    </div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <?php elseif (!empty($gambar_file_dari_db)): ?>
                                        <small class="text-danger" title="File '<?= e($gambar_file_dari_db) ?>' tidak ditemukan di server."><i class="fas fa-image"></i> Gagal</small>
                                    <?php else: ?>
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <a href="<?= e(ADMIN_URL . '/alat_sewa/edit_alat.php?id=' . ($alat['id'] ?? '')) ?>" class="btn btn-warning" title="Edit Alat Sewa">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="<?= e(ADMIN_URL . '/alat_sewa/hapus_alat.php?id=' . ($alat['id'] ?? '')) ?>&csrf_token=<?= e(generate_csrf_token()) ?>"
                                            class="btn btn-danger" title="Hapus Alat Sewa"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="10" class="text-center py-4">
                                <?php if ($search_term): ?>
                                    <p class="mb-2 lead">Tidak ditemukan alat yang cocok dengan kata kunci "<strong><?= e($search_term) ?></strong>".</p>
                                    <a href="<?= e(ADMIN_URL . '/alat_sewa/kelola_alat.php') ?>" class="btn btn-primary btn-sm">
                                        <i class="fas fa-list"></i> Tampilkan Semua Alat
                                    </a>
    ...
    if (typeof showImageModal === 'undefined') {
        function showImageModal(src, title) {
            ...
            if (imagePreviewModalEl && typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                ...
                if (!modalInstance) modalInstance = new bootstrap.Modal(imagePreviewModalEl);
                modalInstance.show();
            } else {
