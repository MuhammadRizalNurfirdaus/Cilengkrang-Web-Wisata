...
// 3. Validasi ketersediaan Model Galeri
if (!class_exists('Galeri') || !method_exists('Galeri', 'getAll')) {
    error_log("FATAL ERROR di kelola_galeri.php: Komponen Galeri (Model) tidak tersedia.");
...
try {
    ...
    $foto_list = Galeri::getAll($search_term, 'uploaded_at DESC');

    if ($foto_list === false) {
        $foto_list = [];
        $db_error_detail = Galeri::getLastError() ?: "Periksa log server.";
        $error_galeri = "Gagal mengambil data galeri. Detail: " . e($db_error_detail);
...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        <form action="<?= e(ADMIN_URL . 'galeri/kelola_galeri.php') ?>" method="GET" class="row g-3 align-items-center">
            <div class="col">
                <div class="input-group">
                    <span class="input-group-text bg-light"><i class="fas fa-search"></i></span>
                    <input type="text" name="search" class="form-control" placeholder="Cari berdasarkan Keterangan atau Nama File..." value="<?= e($search_term ?? '') ?>">
                </div>
            </div>
            <div class="col-auto">
            ...
            <?php if ($search_term): ?>
                <div class="col-auto">
                    <a href="<?= e(ADMIN_URL . 'galeri/kelola_galeri.php') ?>" class="btn btn-outline-secondary">Reset</a>
                </div>
            <?php endif; ?>
        </form>
    </div>
...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        ...
        <span class="badge bg-info rounded-pill"><?= is_array($foto_list) ? count($foto_list) : 0 ?> Foto</span>
    </div>
    <div class="card-body">
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="alert alert-info text-center">
                        <?php if ($search_term): ?>
                            <p class="mb-2 lead">Tidak ditemukan foto yang cocok dengan "<strong><?= e($search_term) ?></strong>".</p>
                            <a href="<?= e(ADMIN_URL . 'galeri/kelola_galeri.php') ?>" class="btn btn-primary btn-sm">Tampilkan Semua Foto</a>
                        ...
                        <?php else: ?>
                            <p class="mb-2 lead text-danger"><i class="fas fa-exclamation-triangle"></i> Gagal memuat data galeri.</p>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
