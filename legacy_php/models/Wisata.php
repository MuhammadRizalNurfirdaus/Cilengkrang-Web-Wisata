...
class Wisata
{
    ...
    public static function getAll(?string $searchTerm = null, string $orderBy = 'nama ASC', ?int $limit = null): array|false
    {
        if (!self::checkDbConnection()) return false;
        self::$last_error = null;
        ...
        // Logika Pencarian
        $sql = "SELECT id, nama, deskripsi, gambar, lokasi, created_at, updated_at FROM " . self::$table_name;

        $params = [];
        $types = '';

        if ($searchTerm !== null && !empty($searchTerm)) {
            $sql .= " WHERE (nama LIKE ? OR deskripsi LIKE ? OR lokasi LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $params[] = $likeTerm;
