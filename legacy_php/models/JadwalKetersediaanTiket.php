...
class JadwalKetersediaanTiket
{
    ...
    public static function getAll(?string $searchTerm = null)
    {
        ...
        $sql = "SELECT jkt.*, 
                       jt.nama_layanan_display, jt.tipe_hari
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            ...
            $date = DateTime::createFromFormat('Y-m-d', $searchTerm);
            if ($date && $date->format('Y-m-d') === $searchTerm) {
                // Search by exact date
                $sql .= " WHERE jkt.tanggal = ?";
                $params[] = $searchTerm;
                $types .= 's';
            } else {
                // Search by ticket name
                $sql .= " WHERE jt.nama_layanan_display LIKE ?";
                $params[] = "%" . $searchTerm . "%";
                $types .= 's';
            }
        }

        $sql .= " ORDER BY jkt.tanggal DESC, jt.nama_layanan_display ASC, jt.tipe_hari ASC";
