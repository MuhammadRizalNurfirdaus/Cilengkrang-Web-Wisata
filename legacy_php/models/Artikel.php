...
class Artikel
{
     ...
     */
     public static function getAll(?string $searchTerm = null, string $orderBy = 'created_at DESC'): array|false
    {
        if (!self::checkDependencies()) return false;

        // --- Validasi Order By (dari kode Anda) ---
        ...
        // Pastikan nama kolom aman sebelum dimasukkan ke query
        $orderBySafe = "`" . str_replace('`', '', $column) . "` " . $direction;
        // --- Akhir Validasi Order By ---


        // --- Logika Pencarian ---
        $sql = "SELECT id, judul, isi, gambar, created_at FROM " . self::$table_name;
        
        $params = [];
        $types = '';

        if ($searchTerm !== null && !empty($searchTerm)) {
            $sql .= " WHERE (judul LIKE ? OR isi LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $types .= 'ss';
        ...
        try {
            ...
            if ($stmt === false) {
                throw new Exception("Gagal mempersiapkan statement SQL: " . self::$db->error);
            }
