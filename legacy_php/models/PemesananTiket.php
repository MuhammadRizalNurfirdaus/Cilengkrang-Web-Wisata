...
class PemesananTiket
{
    ...
    public static function getAll(?string $searchTerm = null, string $orderBy = 'pt.created_at DESC'): array|false
    {
        if (!self::checkDbConnection()) return false;
        self::$last_error = null;

        // Validasi Order By
        $allowed_columns = ['pt.id', 'pt.kode_pemesanan', 'pt.tanggal_kunjungan', 'pt.total_harga_akhir', 'pt.status', 'pt.created_at', 'u.nama_lengkap', 'pt.nama_pemesan_tamu'];
        ...
        // Query dasar dengan JOIN ke tabel users
        $sql = "SELECT pt.id, pt.user_id, pt.kode_pemesanan, pt.tanggal_kunjungan, pt.total_harga_akhir, 
                       pt.status, pt.status AS status_pemesanan, pt.created_at, 
                       pt.nama_pemesan_tamu, pt.email_pemesan_tamu, pt.nohp_pemesan_tamu,
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            // Mencari di kode pemesanan, nama tamu, email tamu, nama user, dan email user.
            $sql .= " WHERE (pt.kode_pemesanan LIKE ? 
                         OR pt.nama_pemesan_tamu LIKE ? 
                         OR pt.email_pemesan_tamu LIKE ?
                         OR u.nama_lengkap LIKE ?
                         OR u.email LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            // Lima placeholder untuk lima kondisi LIKE
        ...
        try {
            ...
            if ($stmt === false) throw new Exception("Gagal mempersiapkan statement: " . self::$db->error);

            if (!empty($params)) $stmt->bind_param($types, ...$params);

            if (!$stmt->execute()) throw new Exception("Eksekusi statement gagal: " . $stmt->error);
            ...
            self::$last_error = "MySQLi Query Error: " . $e->getMessage();
