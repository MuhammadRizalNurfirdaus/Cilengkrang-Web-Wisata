...
class Galeri
{
     ...
     */
    public static function getAll(?string $searchTerm = null, string $orderBy = 'uploaded_at DESC', ?int $limit = null): array|false
    {
        if (!self::checkDbConnection()) return false;
        ...
        // Logika Pencarian
        $sql = "SELECT id, nama_file, keterangan, uploaded_at FROM " . self::$table_name;

        $params = [];
        $types = '';

        if ($searchTerm !== null && !empty($searchTerm)) {
            $sql .= " WHERE (keterangan LIKE ? OR nama_file LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $types .= 'ss';
