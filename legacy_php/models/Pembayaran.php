...
class Pembayaran
{
    ...
    public static function findByPemesananIdAndStatus($pemesanan_tiket_id, $statusArray)
    {
     ...
     public static function findAllWithKodePemesanan(?string $searchTerm = null, string $orderBy = 'p.created_at DESC')
    {
        self::$last_internal_error = null;
        if (!self::checkDbConnection()) return false;

        // Validasi Order By (bisa disederhanakan jika perlu)
        $orderBySafe = $orderBy; // Asumsikan $orderBy aman atau divalidasi di controller
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            $sql .= " WHERE (p.kode_pemesanan LIKE ? 
                         OR COALESCE(u.nama_lengkap, pt.nama_pemesan_tamu) LIKE ?
                         OR p.metode_pembayaran LIKE ?
                         OR p.id_transaksi_gateway LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $params[] = $likeTerm;
        ...
        try {
            ...
            if ($stmt === false) throw new Exception("Gagal prepare: " . self::$db->error);

            if (!empty($params)) $stmt->bind_param($types, ...$params);

            if (!$stmt->execute()) throw new Exception("Gagal execute: " . $stmt->error);
            ...
            self::$last_internal_error = "MySQLi Query Error (findAllWithKodePemesanan): " . $e->getMessage();
