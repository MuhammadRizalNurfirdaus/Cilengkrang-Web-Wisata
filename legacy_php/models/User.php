...
class User
{
    ...
    public static function getAll(?string $searchTerm = null, string $orderBy = 'nama_lengkap', string $orderDir = 'ASC')
    {
        self::$last_internal_error = null;
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            // Search in username, full name, and email
            $sql .= " WHERE (`nama` LIKE ? OR `nama_lengkap` LIKE ? OR `email` LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $params[] = $likeTerm;
        ...
        try {
            ...
            self::$last_internal_error = "Query Error: " . $e->getMessage();
            error_log(get_called_class() . "::getAll() - " . self::$last_internal_error . " | SQL: " . $sql);
            return false;
        }
    }
