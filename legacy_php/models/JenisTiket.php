...
class JenisTiket
{
     ...
     */
    public static function getAll(?string $searchTerm = null, string $orderBy = 'jt.nama_layanan_display ASC, jt.tipe_hari ASC'): array|false
    {
        if (!self::checkDbConnection()) return false;
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            // Mencari di nama layanan, tipe hari, atau nama wisata terkait
            $sql .= " WHERE (jt.nama_layanan_display LIKE ? OR jt.tipe_hari LIKE ? OR w.nama LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $params[] = $likeTerm;
        ...
        $sql .= " ORDER BY " . $orderBy; // Asumsikan $orderBy sudah aman
