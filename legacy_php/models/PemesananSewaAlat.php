...
class PemesananSewaAlat
{
    ...
    // Contoh untuk getAll:
    public static function getAll(?string $searchTerm = null)
    {
        if (!self::checkDbConnection()) return false;

        $sql = "SELECT psa.*, sa.nama_item AS nama_alat, pt.kode_pemesanan AS kode_pemesanan_tiket, 
                       COALESCE(u.nama_lengkap, pt.nama_pemesan_tamu) AS nama_pemesan, 
        ...
        if ($searchTerm !== null && !empty($searchTerm)) {
            // Mencari di nama pemesan (tamu/user), kode tiket, dan nama alat
            $sql .= " WHERE (COALESCE(u.nama_lengkap, pt.nama_pemesan_tamu) LIKE ? 
                         OR pt.kode_pemesanan LIKE ? 
                         OR sa.nama_item LIKE ?)";
            $likeTerm = "%" . $searchTerm . "%";
            $params[] = $likeTerm;
            $params[] = $likeTerm;
            $params[] = $likeTerm;
        ...
        $sql .= " ORDER BY psa.created_at DESC, psa.id DESC";
        ...
        try {
            ...
            error_log(get_called_class() . "::getAll() Query Error: " . $e->getMessage() . " | SQL: " . $sql);
            return false;
        }
    }
